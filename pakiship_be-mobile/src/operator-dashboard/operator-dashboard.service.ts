...
export class OperatorDashboardService {
  ...
  private async listHubParcelRows(hubId: string): Promise<ParcelHubRecordRow[]> {
    ...
    const { data, error } = await admin
      ...
      .limit(100);
    if (error) {
      console.warn("[listHubParcelRows] location schema not accessible yet, returning empty:", error.message);
      return [];
    }
  ...
  private async loadKpiMetrics(hubId: string, dayStart: Date) {
    ...
    if (
      ...
      console.warn("[loadKpiMetrics] location schema not accessible yet, returning zero metrics:", {
  ...
  async getParcels(session: SessionPayload) {
    ...
    const { data: hubInfo } = await admin.schema("location").from("drop_off_points").select("name").eq("id", hubId).maybeSingle();
    const hubName = hubInfo?.name || "";

    let orQuery = `drop_off_point_id.eq.${hubId}`;
    if (hubName) {
      orQuery += `,pickup_address.ilike.%${hubName}%,delivery_address.ilike.%${hubName}%,drop_off_point_name.ilike.%${hubName}%`;
    }
  ...
  async dispatchToDriver(session: SessionPayload, recordId: string) {
    ...
    let updateQuery = admin.schema("location").from("parcel_hub_records")
  ...
  async updateRelayBookingStatus(
    ...
    const currentLocation = String(input.currentLocation ?? (booking.data as any).tracking_current_location ?? "Drop-off point").trim();
    const progressLabel = String(input.progressLabel ?? (booking.data as any).tracking_progress_label ?? "Parcel received at drop-off point").trim();
    ...
    return {
      ...
      currentLocation: (updateResult.data as any).tracking_current_location,
      progressLabel: (updateResult.data as any).tracking_progress_label,
