...
export class ParcelDraftsRepository {
  ...
  async findDraftByTrackingNumber(trackingNumber: string) {
    ...
    const queryByTrackingNumber = (pattern: string) =>
      supabase
        .schema("parcel")
        .from("parcel_drafts")
        .select(selectFields)
    ...
    const exactResult = await queryByTrackingNumber(normalizedTrackingNumber);
    if (exactResult.error || exactResult.data || !normalizedTrackingNumber) {
      return exactResult;
    }

    const escapedTrackingNumber = normalizedTrackingNumber.replace(/[%_]/g, "\\$&");
    return queryByTrackingNumber(`%${escapedTrackingNumber}%`);
  }
  ...
  async updateRelayBookingTracking(draftId: string, patch: Record<string, unknown>) {
    ...
    return supabase
      ...
      .eq("service_id", "pakishare")
      .select(
        "id, tracking_number, tracking_progress_percentage",
      )
      .single();
  }
}
