...
export class ParcelDraftsService {
  ...
  async completeBooking(user: SessionPayload, draftId: string, body: Record<string, unknown>) {
    ...
    if (paymentMethod === "wallet" || paymentMethod === "securepay" || paymentMethod === "bank") {
      ...
      if (
        ...
        if (fullDraft && fullDraft.service_id !== 'pakishare') {
        ...
        } else if (fullDraft && fullDraft.service_id === 'pakishare') {
          console.log(`[completeBooking] PakiShare booking: Skipping immediate driver job creation. Waiting for Operator dispatch.`);
        }
      }
  ...
  async getTrackingDetails(trackingNumber: string) {
    ...
    if (error) {
      console.warn("[getTrackingDetails] Tracking lookup failed:", {
        trackingNumber: normalizedTrackingNumber,
        message: error.message,
      });
      throw new InternalServerErrorException("Unable to fetch tracking details.");
    }
    ...
    try {
      ...
      const { data: job, error: jobError } = await this.supabaseService.createAdminClient()
        ...
        .select("id, job_number, parcel_draft_id, driver_id, status, earnings")
        .eq("job_number", data.tracking_number)
        .maybeSingle();

      if (jobError) {
        console.warn("[getTrackingDetails] Driver job lookup failed:", {
          trackingNumber: data.tracking_number,
    ...
    // Fetch real-time tracking events from the location schema
    let trackingEvents: any[] = [];
    if (data.id) {
      try {
        const { data: events, error: eventsError } = await this.supabaseService.createAdminClient()
          .schema("location")
          .from("parcel_tracking_events")
          .select("*")
          .eq("parcel_draft_id", data.id)
          .order("occurred_at", { ascending: true });

        if (eventsError) {
          console.warn("[getTrackingDetails] Failed to fetch tracking events:", eventsError.message);
        } else if (events) {
          trackingEvents = events;
        }
      } catch (err) {
        console.warn("[getTrackingDetails] Failed to query tracking events:", err.message);
      }
    }

    const findEvent = (statusName: string) => {
      return trackingEvents.find(e => e.status?.toUpperCase() === statusName.toUpperCase());
    };

    const acceptedEvent = findEvent("ACCEPTED");
    const pickedUpEvent = findEvent("PICKED_UP");
    const outForDeliveryEvent = findEvent("OUT_FOR_DELIVERY");
    const deliveredEvent = findEvent("DELIVERED");

    const assignedDriverId = jobDetails?.driver_id || (data as any).assigned_driver_id;

    let statusLabel = 
      data.status === "submitted" ? "Booking Confirmed" : 
      data.status === "accepted" ? "Preparing for Pickup" :
      data.status === "picked-up" || data.status === "picked_up" ? "In Transit" :
      data.status === "out-for-delivery" || data.status === "out_for_delivery" ? "In Transit" :
    ...
    if (jobDetails) {
      ...
      if (s === "completed" || s === "delivered") {
        ...
        } else if (data.status === "picked-up" || data.status === "picked_up" || data.status === "out-for-delivery" || data.status === "out_for_delivery") {
          statusLabel = "In Transit";
        }
      }
    }

    const progress = Number((data as any).tracking_progress_percentage ?? 0);
    const isDelivered = data.status === "delivered" || jobDetails?.status === "completed" || Boolean(deliveredEvent);
    const bookingTime = formatTrackingTime((data as any).created_at ?? null);
    const updateTime = formatTrackingTime((data as any).updated_at ?? null);
    const activeTime = updateTime === "Pending" ? bookingTime : updateTime;
    const pickupDone = Boolean(assignedDriverId) || progress >= 25 || isDelivered;
    const transitDone = progress >= 50 || isDelivered;
    ...
    const pickupLat = parseCoordinate((data as any).pickup_lat);
    const pickupLng = parseCoordinate((data as any).pickup_lng);
    const deliveryLat = parseCoordinate((data as any).delivery_lat);
    const deliveryLng = parseCoordinate((data as any).delivery_lng);
      ...
      timeline: [
        {
          ...
          timestamp: bookingTime === "Pending" ? "Confirmed" : bookingTime,
          completed: true,
        },
        {
          status: "Preparing for Pickup",
          location: acceptedEvent?.location_label || data.pickup_address,
          timestamp: acceptedEvent ? formatTrackingTime(acceptedEvent.occurred_at) : (pickupDone ? activeTime : "Pending"),
          completed: Boolean(acceptedEvent || pickupDone),
        },
        {
          status: "In Transit",
          location: outForDeliveryEvent?.location_label || pickedUpEvent?.location_label || data.delivery_address,
          timestamp: outForDeliveryEvent ? formatTrackingTime(outForDeliveryEvent.occurred_at) : 
                     (pickedUpEvent ? formatTrackingTime(pickedUpEvent.occurred_at) : 
                     (transitDone ? activeTime : "Pending")),
          completed: Boolean(outForDeliveryEvent || pickedUpEvent || transitDone),
        },
        {
          status: "Delivered",
          location: deliveredEvent?.location_label || data.delivery_address,
          timestamp: deliveredEvent ? formatTrackingTime(deliveredEvent.occurred_at) : (isDelivered ? activeTime : "Pending"),
          completed: Boolean(deliveredEvent || isDelivered),
        },
      ],
    };
  }
  ...
  async getHistory(user: SessionPayload) {
    ...
    return {
      transactions: (data ?? []).map((draft) => {
        const createdAt = draft.parcel_draft_items?.[0]?.created_at ?? null;
  ...
  async getHistoryDetails(user: SessionPayload, trackingNumber: string) {
    ...
    const createdAt = data.parcel_draft_items?.[0]?.created_at ?? null;
    const updatedAt = data.parcel_draft_items?.[0]?.created_at ?? null;
        ...
        timeline: [
          {
            ...
            time: formatHistoryDate(createdAt),
            location: data.pickup_address,
            completed: true,
          },
          {
            status: historyStatus.label,
            time: formatHistoryDate(updatedAt),
  ...
  async confirmPayment(user: SessionPayload, draftId: string, body: Record<string, unknown>) {
    ...
    if (fullDraft && fullDraft.service_id !== 'pakishare') {
      ...
      console.log(`[confirmPayment] Found direct draft with tracking: ${fullDraft.tracking_number}. Calling createJobFromDraft...`);
      try {
        await this.driverDashboardService.createJobFromDraft(
          fullDraft,
          items || []
      ...
      console.log(`[confirmPayment] PakiShare booking: Skipping immediate driver job creation. Waiting for Operator dispatch.`);
    }
