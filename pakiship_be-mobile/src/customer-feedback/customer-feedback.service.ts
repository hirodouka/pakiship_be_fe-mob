    ...
    if (ownedParcel.assigned_driver_id) {
      try {
        await admin
          ...
          .update({ rating: input.rating } as any)
          .eq("parcel_draft_id", ownedParcel.id)
          .eq("driver_id", ownedParcel.assigned_driver_id);
      } catch (err) {
        console.warn('[submitFeedback] Non-critical driver job rating update failed:', err.message);
      }
    }
