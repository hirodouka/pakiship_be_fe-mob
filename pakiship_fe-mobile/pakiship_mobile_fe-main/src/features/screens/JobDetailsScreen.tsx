...
import MapPreview from '../parcel/components/MapPreview';
...
export default function JobDetailsScreen() {
  ...
  const fetchJobDetail = async () => {
    try {
      ...
      const mappedJob = {
        ...
        parcelStatus: res.parcelStatus || null,
      };
      setJob(mappedJob);
  ...
  const handleCancelJob = async () => {
    try {
      await apiRequest(`/pakiship/mobile/driver/jobs/${job.id}/cancel`, { method: 'POST' });
      setShowCancelModal(false);
      Alert.alert('Job Cancelled', 'The job has been successfully cancelled.', [{ text: 'OK' }]);
      navigation.goBack();
    } catch (error: any) {
      console.log('Failed to cancel job:', error?.message || error);
      setShowCancelModal(false);
      Alert.alert('Cancel Job Failed', error?.message || 'An error occurred.', [{ text: 'OK' }]);
    }
  };
      ...
      <View style={styles.screen}>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.card}>
            ...
            <View style={[styles.mapCard, { padding: 0, overflow: 'hidden', height: 200, marginBottom: 12 }]}>
              <MapPreview 
                pickupLocation={job.pickupLocation} 
                deliveryLocation={job.dropoffLocation}
                pickupAddress={job.pickup}
                deliveryAddress={job.dropoff}
              />
            </View>
          ...
          {status === 'in-progress' ? (
            <View style={{ gap: 12 }}>
              ...
              {job.parcelStatus !== 'picked-up' && job.parcelStatus !== 'out-for-delivery' && job.parcelStatus !== 'delivered' && (
                <Pressable style={styles.cancelActionButton} onPress={() => setShowCancelModal(true)}>
...
function CancelJobModal({
  ...
  return (
    <View style={styles.modalOverlay}>
      ...
      <View style={styles.acceptModalCard}>
        <View style={styles.acceptMascotWrap}>
          <MaterialCommunityIcons name="alert-circle-outline" size={60} color={palette.danger} />
        </View>

        <Text style={styles.acceptTitle}>Cancel Job?</Text>
        <Text style={styles.acceptSubtitle}>
          Are you sure you want to cancel <Text style={styles.acceptTag}>{jobTag}</Text>? This will negatively affect your acceptance rate.
        </Text>

        <View style={styles.acceptActions}>
          <Pressable style={styles.acceptCancelButton} onPress={onCancel}>
            <Text style={styles.acceptCancelText}>Go Back</Text>
          </Pressable>
          <Pressable style={[styles.acceptConfirmButton, { backgroundColor: palette.danger }]} onPress={onConfirm}>
            <Text style={styles.acceptConfirmText}>Yes, Cancel Job</Text>
          </Pressable>
        </View>
      </View>
    </View>
