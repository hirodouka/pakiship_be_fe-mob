...
export default function TrackPackage() {
  ...
  const fetchTrackingData = async (id: string) => {
    const normalizedTrackingNumber = id.trim().toUpperCase();
    ...
    try {
      const res = await parcelApi.getTrackingDetails(normalizedTrackingNumber);
      if (res) {
        setTrackingResult(res);
      } else {
        setError('Parcel not found for that tracking number.');
      }
    } catch (err: any) {
      ...
      setError(
        isNotFound
          ? 'Parcel not found for that tracking number.'
          : 'Unable to fetch tracking details. Please check your connection and try again.',
      );
    } finally {
  ...
  const fetchTrackingDataQuietly = async (id: string) => {
    try {
      const res = await parcelApi.getTrackingDetails(id.trim().toUpperCase());
      if (res) {
        setTrackingResult(res);
      }
    } catch (err) {
              ...
              {(trackingResult.timeline ?? []).map((event: any, i: number) => (
                <View key={i} style={styles.timelineItem}>
                  <View style={styles.timelineLeft}>
                    ...
                    {i < (trackingResult.timeline?.length ?? 0) - 1 && <View style={[styles.timelineLine, event.completed && styles.timelineLineDone]} />}
                  </View>
          ...
          {trackingResult.assignedDriver && (
            <View style={styles.driverCard}>
              <View>
                ...
                <Text style={styles.driverName}>{(trackingResult.assignedDriver.name || 'Driver').split(' ').map((s: string) => s.charAt(0).toUpperCase() + s.slice(1).toLowerCase()).join(' ')}</Text>
                <Text style={styles.driverMeta}>
                  {trackingResult.assignedDriver.vehicleType || 'Pending dispatch'} · {trackingResult.assignedDriver.plateNumber || 'TBD'}
                </Text>
              </View>
              <TouchableOpacity
