...
async function run() {
  try {
    ...
    const customRpcs = paths.filter(p => p.startsWith('/rpc/') && !p.startsWith('/rpc/st_') && !p.startsWith('/rpc/postgis_') && !p.startsWith('/rpc/_st_'));
    console.log('Custom RPCs in public:', customRpcs);
  } catch (err) {
